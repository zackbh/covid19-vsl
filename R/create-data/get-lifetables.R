###############################################################################
# Country-level life expectancy at each age -----
## Produces "life-tables.RDS"
## Life exptectancy for each country-age and avg for working age
## Data from WHO
...
read_lifetables <- function(filename){
  ...
  out <- df %>% select(indicator, age_group, male_2016, female_2016) %>%
    ...
    mutate(age_group = case_when(age_group %in% c("&lt;1 year", "<1 year", "1-4 years" ) ~ "0-4",
                                 ...
                                 TRUE ~ NA_character_),  # If unmatched
    ...
    mutate(country = stringr::str_split(filename, pattern = ".csv")[[1]][1],
           ...
           age_group = forcats::fct_relevel(forcats::as_factor(age_group),
...
working_life_exp <- df %>%  # Define working age as 20-64 
